...
radioPanelCheckbox.addEventListener("click", async () => {
	...
	try {
		...
		await chrome.tabs.sendMessage(tab.id, { action: "hide-radio-panel", radioPanelStatus: radioState})
	} catch (e) {
		console.error(e)
	}
})
...
stopRadioElement.addEventListener("click", async () => {
	...
	try {
		const [tab] = await chrome.tabs.query({ active: true, lastFocusedWindow: true})
		await chrome.tabs.sendMessage(tab.id, { action: "radio-player-click", radioPlayerStatus: radioPlayerStatus})
	} catch (e) {
		console.error(e)
	}
})
